...
int main()
{
    ...
    // Create cpuset for both threads, lock to core 0
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
    CPU_SET(0, &cpuset);
    ...
    char *iden = "A";
    pthread_create(&thread1, NULL, (void *)print_number, iden);
    // Lock thread A to core 0
    ret = pthread_setaffinity_np(thread1, sizeof(cpu_set_t), &cpuset);

    // Create and start thread B, pass identifier to thread
    iden = "B";
    pthread_create(&thread2, NULL, (void *)print_number, iden);
    // Lock thread B to core 0
    ret = pthread_setaffinity_np(thread2, sizeof(cpu_set_t), &cpuset);
